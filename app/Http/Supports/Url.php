...
class Url
{
    ...
    protected static function _getAllParams($queryString) // string => arr with _o is end of element
    {
        ...
        $tmp = arrayGet($arrQueryStr, self::QUERY, false);
        if ($tmp) {
            $keys = array_keys($arrQueryStr);
            if (arrayGet($keys, 0, '') == self::QUERY) {
    ...
    public static function getBackUrl($urlDefault = null)
    {
        ...
        $listBackUrl = arrayGet($allSession, '_backUrl', []);
        ...
        if (!$oldKey) {
            if (is_null($urlDefault)) {
                ...
                $urlDefault = backendBuildRouteUrl($module .'.index');
            }
            return $urlDefault;
        }

        return arrayGet($listBackUrl, $oldKey);
    }
